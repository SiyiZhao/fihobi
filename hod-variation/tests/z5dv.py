...
# %%
path_cfg = '/global/homes/s/siyizhao/projects/fihobi/hod-variation/configs/QSO-fnl100/z5_base-dv.yaml'
path_z5 = '/pscratch/sd/s/siyizhao/data_learnCosm/AbacusMocks/QSOz5-fnl30bf/Abacus_pngbase_c302_ph000/z2.500/galaxies_rsd/clustering.npy'
path_z5_dv = '/pscratch/sd/s/siyizhao/data_learnCosm/AbacusMocks/QSOz5-fnl30bf/Abacus_pngbase_c302_ph000/z2.500/galaxies_rsd_dv/clustering.npy'
clus_z5 = np.load(path_z5, allow_pickle=True).item()['QSO_QSO']
clus_z5_dv = np.load(path_z5_dv, allow_pickle=True).item()['QSO_QSO']
...
the_z5['wp']=clus_z5[istart:istart+len(obs['wp'])]
the_z5_dv['wp']=clus_z5_dv[istart:istart+len(obs['wp'])]

istart+=len(obs['wp'])
err_obs['xi0']=errall_d[istart:istart+len(obs['xi0'])]
the_z5['xi0']=clus_z5[istart:istart+len(obs['xi0'])]
the_z5_dv['xi0']=clus_z5_dv[istart:istart+len(obs['xi0'])]

istart+=len(obs['xi0'])
err_obs['xi2']=errall_d[istart:istart+len(obs['xi2'])]        
the_z5['xi2']=clus_z5[istart:istart+len(obs['xi2'])]
the_z5_dv['xi2']=clus_z5_dv[istart:istart+len(obs['xi2'])]
...
for i,ctype in enumerate(ctypes):
    ...
    ## models
    axs[0,i].plot(x,x*the_z5[ctype],color='black',label='z5 base')
    axs[1,i].plot(x,(the_z5[ctype]-obs[ctype])/obs[ctype],color='black')
    axs[0,i].plot(x,x*the_z5_dv[ctype],color='red',label='z5 base-dv')
    axs[1,i].plot(x,(the_z5_dv[ctype]-obs[ctype])/obs[ctype],color='red')
...
fig.savefig('output/test_z5dv.png',dpi=300)