...
# %%
path_cfg = '/global/homes/s/siyizhao/projects/fihobi/hod-variation/configs/QSO-fnl100/z2_base-dv.yaml'
path_z2 = '/pscratch/sd/s/siyizhao/data_learnCosm/AbacusMocks/QSOz2-fnl100bf/Abacus_pngbase_c302_ph000/z1.250/galaxies_rsd/clustering.npy'
path_z2_dv = '/pscratch/sd/s/siyizhao/data_learnCosm/AbacusMocks/QSOz2-fnl100bf/Abacus_pngbase_c302_ph000/z1.250/galaxies_rsd_dv/clustering.npy'
clus_z2 = np.load(path_z2, allow_pickle=True).item()['QSO_QSO']
clus_z2_dv = np.load(path_z2_dv, allow_pickle=True).item()['QSO_QSO']
...
the_z2['wp']=clus_z2[istart:istart+len(obs['wp'])]
the_z2_dv['wp']=clus_z2_dv[istart:istart+len(obs['wp'])]

istart+=len(obs['wp'])
err_obs['xi0']=errall_d[istart:istart+len(obs['xi0'])]
the_z2['xi0']=clus_z2[istart:istart+len(obs['xi0'])]
the_z2_dv['xi0']=clus_z2_dv[istart:istart+len(obs['xi0'])]

istart+=len(obs['xi0'])
err_obs['xi2']=errall_d[istart:istart+len(obs['xi2'])]        
the_z2['xi2']=clus_z2[istart:istart+len(obs['xi2'])]
the_z2_dv['xi2']=clus_z2_dv[istart:istart+len(obs['xi2'])]
...
for i,ctype in enumerate(ctypes):
    ...
    ## models
    axs[0,i].plot(x,x*the_z2[ctype],color='black',label='z2 base')
    axs[1,i].plot(x,(the_z2[ctype]-obs[ctype])/obs[ctype],color='black')
    axs[0,i].plot(x,x*the_z2_dv[ctype],color='red',label='z2 base-dv')
    axs[1,i].plot(x,(the_z2_dv[ctype]-obs[ctype])/obs[ctype],color='red')
...
fig.savefig('output/test_z2dv.png',dpi=300)