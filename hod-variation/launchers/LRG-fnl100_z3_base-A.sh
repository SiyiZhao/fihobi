...
outdir=/pscratch/sd/s/siyizhao/desi-dr2-hod/LRG-fnl100/z3_base-A/
mkdir -p $outdir
...
srun -N 2 -n 4 -c 64 --cpu-bind=cores python scripts/run_pmn.py --config $config > $outdir/run_v2.1.log 2>&1
srun -n 1 -c 64 --cpu-bind=cores python scripts/post.py --config $config > $outdir/post_v2.1.log 2>&1
