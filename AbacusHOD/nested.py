...
# -*- coding: utf-8 -*-
"""
This is a script for running nested sampling on HOD mock catalogs.
Fitting data are the projected correlation function wp of LRG_LRG tracer combo + number density.
...
DEFAULTS = {}
DEFAULTS['path2config'] = 'config/lrg_y3wp.yaml'
...
def lnprob(p, param_mapping, param_tracer, Data, Ball):
    ...
    theory_density['clustering'] = clustering['LRG_LRG']
...
def wp_Data(data_params, HOD_params):
    """
    Read the observed data and covariance matrix for the LRG_LRG tracer combo.
    ---
    Return: RefData, a class instance of ObsData containing the data and covariance matrix.
    """
    ...
    #!!! attention: if you use yuan's data for SV3, the wp should be masked with the reasonable range 
    # # rp_yuan = data[:, 0]
    # wp_yuan = data[:, 1]
    # cov_yuan = np.loadtxt(path2cov)
    # wp = wp_yuan[7:7 + 14]  # select the range of wp to fit
    # cov = cov_yuan[7:7 + 14, 7:7 + 14]  # select the range of covariance to fit
    wp = data[:, 1]  
    cov = np.loadtxt(path2cov)  
    ...
    RefData = ObsData(data=wp, cov=cov, ngal=density_mean, ngal_std=density_std)
    return RefData
